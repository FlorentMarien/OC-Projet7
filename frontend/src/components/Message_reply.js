...
function Message_reply({auth,parametre,messagetarget,listMessage,setListMessage,profilTarget}) {
	...
	function sendAnswer(e,replyLevel=0){
		...
		if(e.target.closest("div.message") === null){
			messagetarget=e.target.closest("div.listAnswer").children[0].attributes["messageid"].value;
		}else{
			messagetarget=e.target.closest("div.message").attributes["messageid"].value;
		}
		objectData={
			...objectData,
			...
			if(listMessage[0]._id === undefined){
				if(parametre.replyLevel===0){
					for(let x=0;x<listMessage.length;x++){
						if(listMessage[x].answerArray[0][0]._id === messagetarget){
	...
	return (
		<div className='listAnswer_sendreply'>
			...
			<IconButton onClick={(e)=>{setopenReply(1)}} color="primary" aria-label="Add commentary" component="label">
				<AddCommentIcon />
			</IconButton>
			:
				...
				<ThemeProvider theme={theme}>
					...
					<Button color="primary" variant="contained" onClick={(e)=>setopenReply(0)}>Close</Button>
					</div>
				</ThemeProvider>
