    ...
    socket.on('message', (msg) => {
        ...
        for (let u in users) {
            if (u === objectmessage.destuserId || u === objectmessage.userId) {
                ...
                if (
                    u === objectmessage.destuserId &&
                    objectmessage.state === 'close'
                ) {
                    users[u].send(message);
                }
                if (objectmessage.state === 'sendfile') {
                    if (u === objectmessage.userId) {
                        Privatemessage.sendMessage(objectmessage);
                    }
                    users[u].send(message);
                }
                if (
                    objectmessage.state === 'send' &&
                    u === objectmessage.userId
                ) {
                    Privatemessage.sendMessage(objectmessage);
                }
                if (
                    objectmessage.state === 'isWrite' &&
                    u ===
                        objectmessage.destuserId /* Seulement destuserid a ajouter*/
                ) {
                    users[u].send(message);
